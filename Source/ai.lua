...
--   · Medium  – like Easy but picks the *best* safe edge via a depth‑2 local heuristic.
--   · Hard    – plays as Medium until few safe edges remain, then runs a
--                negamax chain/loop solver.
--   · Expert  – plays like Medium, but watches for small pockets or any chain
--                to flip into full Berlekamp Nim‑sum + double‑cross.
...
-- Flood-fill free-edge regions (Expert pockets) -----------------------------
...
local function chooseEasyMove(board)
  ...
  local safeList = safeEdges(board)
  if #safeList > 0 then
    return safeList[math.random(#safeList)]
  end
  local free = listFreeEdges(board)
  return free[math.random(#free)]
end
...
local function chooseHardMove(board)
  ...
  if #closers > 0 then return closers[1] end
  local safeList = safeEdges(board)
  if #safeList > board.DOTS * board.DOTS / 2 then
    return bestSafeEdge(board, safeList)
  end
  return chooseNegamaxMove(board)
end

local function chooseExpertMove(board)
    -- are we already split into >1 region?  (or has any chain appeared?)
    local regions = freeEdgeRegions(board)
    if #regions > 1 or #collectHotComponents(board) > 0 then
      -- collect *all* the chains & loops across the entire board
      local comps = collectHotComponents(board)
      -- if there are no true chains, comps=={}, so we just play safe edges
      if #comps == 0 then
        local s = safeEdges(board)
        if #s > 0 then return bestSafeEdge(board, s) end
        return listFreeEdges(board)[1]
      end
      -- now do the one‐time global Nim‐sum on comps
      return chooseBerlekampMove(board, comps)
    end
  
    -- still “wide‑open” single region with no 3‑sided boxes—play Medium
